...
- 所有编辑（新增/修改联系人、删除联系人、导出 YAML）发生在**浏览器内存**中，刷新页面即恢复服务器数据。删除操作也只作用于当前网页，不会修改后台 `config/contacts.yaml`。
- 真正的持久化变更由管理员在服务器上手动替换 `config/contacts.yaml` 完成。

当前内置版本号：`20260919-V8`（见 `version.go`）。
...
例如当前版本 `20260919-V8` 会对应 Release 标签 `v20260919-V8`。同一版本后续普通提交不会重复创建 Release；需要重新发布同一版本时，可在 Actions 页面手动执行 `Run workflow` 并填写对应的 `release_tag`。
